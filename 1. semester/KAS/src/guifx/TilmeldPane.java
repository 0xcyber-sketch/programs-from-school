...
public class TilmeldPane extends GridPane {
	...
	private void tilmeldAction() {
		...
		if (konference != null) {
			if (dpAnkomstDato.getValue() != null && dpAfrejseDato.getValue() != null) {
				if (!dpAnkomstDato.getValue().isAfter(dpAfrejseDato.getValue())) {
					...
					if (!deltagerNavn.isEmpty()) {
						...
						if (!adresse.isEmpty()) {
							...
							if (!by_Land.isEmpty()) {
								...
								if (!tlfNr.isEmpty()) {
									String speciale = txfSpeciale.getText().trim();
									ArrayList<Foredrag> foredrager = new ArrayList<Foredrag>(lvwForedrag.getSelectionModel().getSelectedItems());
									if (!chbForedragsholder.isSelected() || (chbForedragsholder.isSelected() && !speciale.isEmpty() && !foredrager.isEmpty())) {
										...
										if (!chbLedsager.isSelected() || (chbLedsager.isSelected() && !txfLedsagerNavn.getText().trim().isEmpty())) {
											...
											setError("Indtast ledsager navn");
										}
									} else {
										setError("Vælg speciale \nog foredrag");
									}
								} else {
									setError("Indtast tlf nr");
								}
							} else {
								setError("Indtast by/land");
							}
						} else {
							setError("Indtast adresse");
						}
					} else {
						setError("Indtast navn");
					}
				} else {
					setError("Fejl i dato valg");
				}
			} else {
				setError("Vælg datoer");
			}
		} else {
			setError("Vælg konference");
		}
	}
	...
	public void restFields() {
		...
		dpAnkomstDato.setValue(null);
		lblError.setVisible(false);
		lvwKonferencer.getSelectionModel().select(null);
		disableTilmelding(true);
	}
	...
	public void updateControls(){
		...
		if (preTilmelding != null) {
			...
			if (preTilmelding.getLedsager() != null) {
				...
				for (Udflugt udflugt : preTilmelding.getLedsager().getUdflugter()) {
